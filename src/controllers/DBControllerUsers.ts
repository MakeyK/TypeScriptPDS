...
class DBControllerUsers {
    async createUsers(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            const { login, password, role }: { login: string; password: string; role?: string } = req.body;
            if (!login || !password) {
    ...
    async DelId(req: Request, res: Response): Promise<Response> {
        try {
    ...
    async DelFull(req: Request, res: Response): Promise<Response> {
        try {
            ...
            if (test_del_all) return res.send({ message: "Все записи удалены!" });
    ...
    async RedId(req: Request, res: Response): Promise<Response> {
        const { id_user }: { id_user?: string; title?: string } = req.body;
    ...
    async updateUser(req: Request, res: Response, next: NextFunction): Promise<void> {
        try {
            ...
            const id_user = decoded.id_user as string;
